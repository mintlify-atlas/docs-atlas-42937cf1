...
```typescript
await client.chat.getChat(guid, options?): Promise<ChatResponse>
```
...
// Get basic chat info
const chat = await client.chat.getChat('iMessage;+;chat123456');
console.log(`Chat name: ${chat.displayName}`);

// Get chat with participants
const chatWithParticipants = await client.chat.getChat('iMessage;+;chat123456', {
  with: ['participants']
});
console.log('Participants:', chatWithParticipants.participants);

// Get chat with participants and messages
const fullChat = await client.chat.getChat('iMessage;+;chat123456', {
...
```typescript
await client.chat.getChatMessages(chatGuid, options?): Promise<MessageResponse[]>
```
...
const recentMessages = await client.chat.getChatMessages(chatGuid, {
  limit: 50,
  sort: 'DESC'
});

// Get messages with pagination
const page1 = await client.chat.getChatMessages(chatGuid, {
  offset: 0,
  limit: 25,
  sort: 'ASC'
});
...
const messages = await client.chat.getChatMessages(chatGuid, {
  after: lastWeek,
  sort: 'DESC'
});

// Get messages with attachments
const messagesWithAttachments = await client.chat.getChatMessages(chatGuid, {
  with: ['attachments'],
  limit: 100
});
